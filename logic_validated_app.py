...
# (Here we use selectbox columns for some fields.)
column_config_main = {}
...
if "Status" in df_main.columns:
    # Default options are now "Not Started", "In Progress", and "Finished"
    column_config_main["Status"] = st.column_config.SelectboxColumn(
        "Status", options=["Not Started", "In Progress", "Finished"], help="Status"
    )
...
# Ensure Status is a string and default if blank.
if "Status" in edited_df_main.columns:
    edited_df_main["Status"] = edited_df_main["Status"].astype(str).fillna("Not Started")

# ---------------------------------------------------------------------
# SAVE UPDATES WITH LOGICAL VALIDATIONS
# ---------------------------------------------------------------------
if st.button("Save Updates (Main Timeline)"):
    validation_error = False
    error_messages = []
    # Iterate over each row and enforce logical consistency.
    for idx, row in edited_df_main.iterrows():
        # 1. Default status if blank.
        status = str(row.get("Status", "")).strip()
        if not status:
            edited_df_main.at[idx, "Status"] = "Not Started"
            status = "Not Started"
        
        # 2. Ensure progress is a number between 0 and 100.
        try:
            progress = float(row.get("Progress", 0))
        except (ValueError, TypeError):
            progress = 0
        progress = max(0, min(100, progress))
        edited_df_main.at[idx, "Progress"] = progress

        status_lower = status.lower()
        # 3. Enforce consistency:
        if status_lower == "finished":
            # If finished, force progress to 100.
            edited_df_main.at[idx, "Progress"] = 100
        elif status_lower == "not started":
            # Not started tasks must have 0 progress.
            edited_df_main.at[idx, "Progress"] = 0
        else:
            # For in-progress tasks:
            if progress == 0:
                edited_df_main.at[idx, "Status"] = "Not Started"
            elif 0 < progress < 100:
                edited_df_main.at[idx, "Status"] = "In Progress"
            elif progress == 100:
                edited_df_main.at[idx, "Status"] = "Finished"

        # 4. Validate dates: Start Date must come before End Date.
        if "Start Date" in edited_df_main.columns and "End Date" in edited_df_main.columns:
            start_date = edited_df_main.at[idx, "Start Date"]
            end_date = edited_df_main.at[idx, "End Date"]
            if pd.notnull(start_date) and pd.notnull(end_date) and start_date > end_date:
                validation_error = True
                error_messages.append(f"Row {idx}: Start Date cannot be after End Date.")
        
        # 5. Optional: If a task was originally finished (in df_main) but now has been modified,
        # warn the user and revert changes.
        if idx < len(df_main) and df_main.at[idx, "Status"].lower() == "finished":
            if status_lower != "finished" or progress != 100:
                st.warning(f"Row {idx} is marked as Finished. Its status and progress are locked.")
                edited_df_main.at[idx, "Status"] = "Finished"
                edited_df_main.at[idx, "Progress"] = 100
    ...
    if validation_error:
        for msg in error_messages:
            st.error(msg)
        st.stop()  # Prevent saving if there are validation errors.
    
    try:
        edited_df_main.to_excel(DATA_FILE, index=False)
        st.success("Main timeline data successfully saved with logical validations!")
        load_timeline_data.clear()
    except Exception as e:
...
def norm_unique(df_input: pd.DataFrame, col: str):
    """Return sorted unique normalized (lowercase stripped) values from a column."""
    if col not in df_input.columns:
        return []
    return sorted(set(df_input[col].dropna().astype(str).str.lower().str.strip()))

# Initialize filter session state values.
...
# Use a default date range computed from the data (but allow the user to change it)
default_date_range = (
...
if "Start Date" in df_filtered.columns and "End Date" in df_filtered.columns:
    srange, erange = selected_date_range  # dates from widget
...
# Ensure all needed columns exist & have the correct types.
