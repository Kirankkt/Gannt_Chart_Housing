...
# ---------------------------------------------------------------------
# 1. HELPER: LOAD MAIN TIMELINE DATA FROM POSTGRESQL
# ---------------------------------------------------------------------
@st.cache_data
def load_timeline_data() -> pd.DataFrame:
    query = f"SELECT * FROM {TIMELINE_TABLE}"
    df = pd.read_sql(query, engine)
    df.columns = df.columns.str.strip()  # Remove any trailing spaces
    # Rename columns to match the expected names in the dashboard
    df.rename(columns={
        "activity": "Activity",
        "item": "Item",
        "task": "Task",
...
def save_timeline_data(df: pd.DataFrame):
    try:
        ...
        return True, "Timeline data saved successfully!"
...
if st.button("Save Updates (Main Timeline)"):
    ...
    success, msg = save_timeline_data(edited_df_main)
    if success:
        st.success("Main timeline data successfully saved!")
        load_timeline_data.clear()
    else:
        st.error(f"Error saving main timeline: {msg}")
...
# =====================================================================
# 8. SECOND TABLE: ITEMS TO ORDER (POSTGRESQL)
# =====================================================================
@st.cache_data
def load_items_data() -> pd.DataFrame:
    query = f"SELECT * FROM {ITEMS_TABLE}"
    df_i = pd.read_sql(query, engine)
    ...
    for needed_col in ["Item", "Quantity", "Order Status", "Delivery Status", "Notes"]:
        if needed_col not in df_i.columns:
            df_i[needed_col] = ""
    df_i["Item"] = df_i["Item"].astype(str)
    df_i["Quantity"] = pd.to_numeric(df_i["Quantity"], errors="coerce").fillna(0).astype(int)
    df_i["Order Status"] = df_i["Order Status"].astype(str)
    df_i["Delivery Status"] = df_i["Delivery Status"].astype(str)
    df_i["Notes"] = df_i["Notes"].astype(str)
    return df_i
...
def save_items_data(df: pd.DataFrame):
    try:
        ...
        return True, "Items data saved successfully!"
    except Exception as e:
        return False, str(e)
...
if st.button("Save Items Table"):
    try:
        ...
        success, msg = save_items_data(edited_df_items)
        if success:
            st.success("Items table successfully saved to PostgreSQL!")
            load_items_data.clear()
        else:
            st.error(f"Error saving items table: {msg}")
    except Exception as e:
